...
public class IMDBPartitioner {
    ...
    public void partition(String dataGraphFilePath, String savingDirectory, HashMap<DataVertex, Integer> partitionMapping, int time) {
        try {
            FileWriter[] writers = new FileWriter[numberOfPartitions];
            for (int i = 0; i < writers.length; i++) {
                writers[i] = new FileWriter(savingDirectory + "imdb" + i + "-" + time + ".nt");
            }
                    ...
                    try {
                        ...
                        if (temp2.length == 2) {
                            ...
                            if (partitionMapping.containsKey(v)) {
                                ...
                                writers[partitionID].write(line + "\n");
