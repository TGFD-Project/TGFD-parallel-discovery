...
import static Discovery.Util.generateDbpediaTimestampToFilesMap;
...
//        options.addOption("type", true, "generate graphs using type");
        ...
        if (cmd.hasOption("path")) {
            ...
            Path dataSource = Paths.get(path);
            if (!Files.isDirectory(dataSource)) {
                System.out.println(dataSource + " is not a valid directory.");
                return;
            }
        }
    ...
    public static void generateCustomDBpediaBasedOnType(String[] args) {
        ...
        for (int i = 0; i < 3; i++) {
            for (String fileType : fileTypes) {
                ...
                switch (fileType) {
                    case TYPES:
                        typesModels[i].read(input.toUri().toString());
                        break;
                    case OBJECTS:
                        objectsModels[i].read(input.toUri().toString());
                        break;
                    case LITERALS:
                        literalsModels[i].read(input.toUri().toString());
                        break;
                }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            while (stmtIterator.hasNext()) {
                ...
                if (counter % 100000 == 0)
                    System.out.println("Processed " + counter + " statements for 201" + i + TYPES);
            }
...
//                        if (counter % 100000 == 0) System.out.println("Processed "+counter+" statements for 201" + i + OBJECTS);
            ...
            for (int i = 5; i < 8; i++) {
                for (String fileType : Arrays.asList(TYPES, LITERALS)) {
                    ...
                    switch (fileType) {
                        case TYPES:
                            model = typesModels[i - 5];
                            break;
//                        case OBJECTS -> model = objectsModels[i-5];
                        case LITERALS:
                            model = literalsModels[i - 5];
                            break;
                    }
...
//                final String filePath = path + "/201" + (i + 5) + "/201" + (i + 5) + fileType + ".ttl";
                        ...
                        StmtIterator stmtIterator = model.listStatements();
                        System.out.printf("Did we meke to here?");
                        switch (fileType) {
                            case TYPES:
                                while (stmtIterator.hasNext()) {
                                    Statement stmt = stmtIterator.nextStatement();
                            ...
                            case LITERALS:
                                while (stmtIterator.hasNext()) {
                                    ...
                                    if (stmt.getPredicate().getLocalName().equalsIgnoreCase("literals")) continue;
                                    String subjectURI = stmt.getSubject().getURI().toLowerCase();
                                    vertexURIToStmtsMap.putIfAbsent(subjectURI, new HashSet<>());
                                    vertexURIToStmtsMap.get(subjectURI).add(stmt);
                                }
...
//                        total += (entry.getValue().size() * percent);
            ...
            for (int j = 0; j < sizes.length; j++) {
                long size = sizes[j];
                String directoryStructure = "dbpedia-" + size + "/201" + (i + 4) + "/";
                String newFileName = directoryStructure + "201" + (i + 4) + "-" + size + ".ttl";
